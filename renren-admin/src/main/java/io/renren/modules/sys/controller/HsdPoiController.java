...
public class HsdPoiController {
    ...
    public R findAll(@RequestParam Map<String, Object> params ){
        ...
        try {
            all = hsdPoiService.findAll(name, order, pageNum, pageSize);
  ...
  //  @RequiresPermissions("sys:hsdpoi:save")
    ...
    public R update(@RequestParam(value = "filename",required = false) MultipartFile[] files,
        ...
        /*if(picture.length != 0){
            for(String item : picture) {
                    ...
                    hsdPicEntity.setPicture(item);
                    hsdPoiService.insertPicture(hsdPicEntity);
                }
                for(String deletePhoto : listUrl){
                    String replaceUrl = deletePhoto.replace("/renren-admin/statics/images/", "");
                    File file = new File("E:\\idea-workspace\\renren-security\\renren-admin\\src\\main\\resources\\statics\\images\\");
    ...
    public R delete(@RequestBody Long[] ids){
        ...
        for (Long id : ids) {
            ...
            for(HsdPicEntity item : picUrl){
                ...
                String replaceUrl = picture.replace("/renren-admin/statics/images/", "");
                File file = new File("E:\\idea-workspace\\renren-security\\renren-admin\\src\\main\\resources\\statics\\images\\");
    ...
    public R clickDelPic(@RequestBody String picUrl){
        ...
        String replaceUrl = picUrl.replace("/renren-admin/statics/images/", "");
        File file = new File("E:\\idea-workspace\\renren-security\\renren-admin\\src\\main\\resources\\statics\\images\\");
        File[] files = file.listFiles();
        for(File filePath : files){
            if(filePath.getName().equals(replaceUrl)){
